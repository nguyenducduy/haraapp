...
<html>
    <head>
        ...
        <link href="{{ static_url('min/index.php?g=cssCoreAdmin&rev=' ~ config.global.version.css) }}" rel="stylesheet" type="text/css">
        {% block css %}{% endblock %}
        <!--[if lte IE 9]>
    ...
    <body class="fixed-header menu-pin">
        {% include '../../Core/View/Layout/admin-sidebar.volt' %}
        <!-- START PAGE-CONTAINER -->
        <div class="page-container">
            {% include '../../Core/View/Layout/admin-header.volt' %}
            <!-- START PAGE CONTENT WRAPPER -->
            <div class="page-content-wrapper">
                <!-- START PAGE CONTENT -->
                <div class="content">
                {% include '../../Core/View/Layout/admin-jumpbotron.volt' %}
                {% block content %}{% endblock %}
                </div>
                <!-- END PAGE CONTENT -->
                {% include '../../Core/View/Layout/admin-footer.volt' %}
        </div>
        <!-- END PAGE CONTAINER -->
        {% include '../../Core/View/Layout/admin-quickview.volt' %}
        {% include '../../Core/View/Layout/admin-search.volt' %}
        <!-- BEGIN PAGE LEVEL JS -->
        <script type="text/javascript" src="{{ static_url('plugins/boostrapv3/js/bootstrap.min.js') }}"></script>
        <script type="text/javascript" src="{{ static_url('min/index.php?g=jsCoreAdmin&rev=' ~ config.global.version.js) }}"></script>
        {% block js %}{% endblock %}
    </body>
</html>
